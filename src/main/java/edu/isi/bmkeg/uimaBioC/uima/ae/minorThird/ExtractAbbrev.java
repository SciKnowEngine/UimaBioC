package edu.isi.bmkeg.uimaBioC.uima.ae.minorThird;
 ...
 * Biomedical Text A.S. Schwartz, M.A. Hearst; Pacific Symposium on Biocomputing
...
public class ExtractAbbrev {
	...
	//
	private List<StringSpan> accum = new ArrayList<StringSpan>(); // accumulator,
																	// used by
																	// Schwartz's
																	// code
	...
	/*
	 * protected void doAnnotate(MonotonicTextLabels labels) { annotationMode =
	 * true; int k=0; for (Iterator<Span>
	 * i=labels.getTextBase().documentSpanIterator(); i.hasNext(); ) {
	 * accum.clear(); Span doc = i.next(); String s = doc.getDocumentContents();
	 * 
	 * // call Schwartz's code to fill up accum with short,long pairs
	 * extractAbbrPairsFromString(s);
	 * 
	 * // build annotations based on the contents of the accumulator for
	 * (Iterator<StringSpan> j=accum.iterator(); j.hasNext(); ) { StringSpan
	 * shortForm = j.next(); StringSpan longForm = j.next(); Span shortSpan =
	 * doc.charIndexSubSpan(shortForm.lo, shortForm.hi);
	 * log.debug("shortSpan["+shortForm.lo+".."+shortForm.hi+"] of doc: near '"+
	 * doc.getDocumentContents().substring(shortForm.lo,shortForm.hi)+"'");
	 * log.debug("shortForm='"+shortForm.asString()+"' shortSpan='"+shortSpan.
	 * asString()+"'");
	 * 
	 * 
	 * Span longSpan = doc.charIndexSubSpan(longForm.lo, longForm.hi);
	 * labels.addToType(shortSpan, SHORT_FORM_TYPE); labels.addToType(longSpan,
	 * LONG_FORM_TYPE); k++; labels.setProperty( shortSpan, SHORT_FORM_PROP,
	 * Integer.toString(k)); labels.setProperty( longSpan, LONG_FORM_PROP,
	 * Integer.toString(k)); } } annotationMode = false; labels.setAnnotatedBy(
	 * PROVIDED_ANNOTATION ); }
	 */
	...
	private void extractAbbrPairsFromString(String currString) {
		...
		for (int i = 0; i < sentence.length; i++) {
			...
			offset += sentence[i].length() + 3; // the +3 is for the period and
												// the two following spaces
		}
	}
	...
	static private class StringSpan {
		...
		public char charAt(int i) {
			return mySubstring.charAt(i);
		}

		public int indexOf(char ch) {
			return mySubstring.indexOf(ch);
		}

		public int indexOf(char ch, int fromIndex) {
			return mySubstring.indexOf(ch, fromIndex);
		}

		public int indexOf(String s) {
			return mySubstring.indexOf(s);
		}

		public int lastIndexOf(String s) {
			return mySubstring.lastIndexOf(s);
		}

		public int lastIndexOf(String s, int fromIndex) {
			return mySubstring.lastIndexOf(s, fromIndex);
		}
		...
		public StringSpan substring(int newLo, int newHi) {
			return new StringSpan(base, lo + newLo, lo + newHi);
		}

		public StringSpan substring(int newLo) {
			return new StringSpan(base, lo + newLo, hi);
		}
			...
			if (shortForm.length() > 0 || longForm.length() > 0) {
				if (shortForm.length() > 1 && longForm.length() > 1) {
					if ((shortForm.indexOf('(') > -1)
						...
						// shortForm = currSentence.substring(openParenIndex +
						// 1, newCloseParenIndex);
	...
	private void extractAbbrPair(StringSpan shortForm, StringSpan longForm) {
		...
		for (int i = shortFormSize - 1; i >= 0; i--)
			if (!Character.isLetterOrDigit(shortForm.charAt(i)))
				shortFormSize--;
		if (bestLongForm.length() < shortForm.length() || bestLongForm.indexOf(shortForm.asString() + " ") > -1
				|| bestLongForm.asString().endsWith(shortForm.asString()) || longFormSize > shortFormSize * 2
